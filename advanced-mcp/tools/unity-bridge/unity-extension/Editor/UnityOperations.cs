...
namespace UnityBridge
{
    public static class UnityOperations
    {
        ...
        private static string GenerateFullCodeForExecution(string userCode)
        {
            ...
            // Простая state-машина для корректного парсинга блоков кода
            int braceLevel = 0;
            ...
            foreach (var line in codeLines)
            {
                ...
                // 1. Собираем все using-директивы
                ...
                // 2. Находим начало определения класса на верхнем уровне
                if (braceLevel == 0 && (trimmedLine.StartsWith("public class") || trimmedLine.StartsWith("class ")))
                {
                    inClassDefinition = true;
                }

                // 3. Добавляем строку в соответствующий блок кода
                ...
                {
                    // Все, что не является классом верхнего уровня, считается исполняемым кодом.
                    // Это предотвращает неверную интерпретацию 'else if', 'for', и т.д.
                    executableCode.AppendLine(line);
                }

                // 4. Отслеживаем уровень вложенности скобок, чтобы найти конец блока класса
                if (line.Contains("{"))
                {
                    braceLevel++;
                }
                ...
                if (line.Contains("}"))
                {
                    ...
                    {
                        // Мы достигли конца текущего определения класса
            ...
            // Ключевое изменение: мы больше не пытаемся выделить "локальные функции".
            // Именно это было источником ошибки парсинга.
...
{{
    ...
    {{
        {executableCode}
        // Добавляем return по умолчанию, если скрипт его не содержит.
